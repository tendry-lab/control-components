...
uint64_t BitOps::mask_u64(uint8_t pos) {
    return (1ULL << pos);
}

uint64_t BitOps::umask_u64(uint8_t pos) {
    return ~(mask_u64(pos));
}
...
uint8_t BitOps::nth_u64(uint64_t value, uint8_t pos) {
    return mask_u64(pos) & value ? 1 : 0;
}
