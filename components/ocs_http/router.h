...
public:
    ...
    void add(IRequest::Method method, const char* pattern, IHandler& handler) override;

    //! Receive a registered handler.
    IHandler* match(IRequest::Method method,
                    const char* pattern,
                    size_t match_upto,
                    IPatternMatcher& matcher) override;

    //! Iterate over each registered handler.
    void for_each(IRequest::Method method, IPatternIterator& iterator) override;

private:
    using Endpoint = std::pair<std::string, IHandler*>;
    using EndpointList = std::vector<Endpoint>;

    std::unordered_map<IRequest::Method, EndpointList> method_to_endpoints_;
};
